        ...
        for connection in self.active_connections[project_id]:
            if connection != exclude:
                try:
                    ...
                    logging.debug(
                        f"WebSocket send failed, marking connection as dead: {type(e).__name__}"
                    )
...
async def invite_member(project_id: str, user_id: str, name: str, email: str, role: str = "viewer"):
    """
    📧 **Invite Team Member**
...
async def add_comment(
    ...
    await manager.broadcast_to_project(
        project_id,
        {"type": "new_comment", "comment": comment.dict(), "timestamp": datetime.now().isoformat()},
    )
...
async def get_activity_feed(project_id: str, limit: int = 50):
    ...
    return ActivityFeed(activities=activities[:limit], total_count=len(activities), unread_count=2)


@router.post("/projects/{project_id}/changes/track")
async def track_change(
    project_id: str, user_id: str, user_name: str, change_type: str, description: str, details: Dict
):
