...
def _check_oib_rl_6(bundesland: str, bgf_m2: float) -> ComplianceResult:
    ...
    checks = [
        ...
        },
        {
            "check": "U-Wert Dach",
            "status": "pass",
            "details": "U ≤ 0,15 W/m²K (Neubau)"
        },
        {
            "check": "U-Wert Fenster",
            "status": "pass",
            "details": "Uw ≤ 1,0 W/m²K (Neubau)"
        },
        {
...
async def generate_compliance_report(
    ...
    passed = sum(
        len([c for c in r.checks if c["status"] == "pass"])
        for r in oib_results
    )
    warnings = sum(
        len([c for c in r.checks if c["status"] == "warning"])
        for r in oib_results
    )
    failed = sum(
        len([c for c in r.checks if c["status"] == "fail"])
        for r in oib_results
    )
