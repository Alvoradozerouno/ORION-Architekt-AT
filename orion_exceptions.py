...
class DataFreshnessError(ValidationError):
    ...
    def __init__(self, age_days, threshold_days=180):
        ...
        super().__init__(f"Data is {age_days} days old (threshold: {threshold_days} days)")
...
class BarrierefreiheitError(ComplianceError):
    ...
    def __init__(self, mangel_list):
        ...
        super().__init__(f"Barrierefreiheit nicht erfüllt: {', '.join(mangel_list)}")
...
class FluchwegError(ComplianceError):
    ...
    def __init__(self, mangel_list):
        self.mangel_list = mangel_list
        super().__init__(f"Fluchtweg-Anforderungen nicht erfüllt: {', '.join(mangel_list)}")
...
def require_field(data, field_name, expected_type=None):
    ...
    if expected_type and not isinstance(value, expected_type):
        raise InvalidDataError(field_name, value, expected_type=expected_type.__name__)
