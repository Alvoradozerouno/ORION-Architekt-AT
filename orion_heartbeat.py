...
class OrionHeartbeat:
    ...
    def _load_state(self):
        ...
        if HEARTBEAT_STATE.exists():
            try:
                ...
                logger.warning(
                    f"Failed to load heartbeat state ({type(e).__name__}): {str(e)[:100]}"
                )
                pass
    ...
    def _task_self_reflection(self) -> Dict:
        ...
        try:
            ...
            reflection = SELF_REFLECTION.reflect_on_decision(
                decision="heartbeat_reflection", reasoning=f"Autonomous pulse #{self.pulse_count}"
            )
                    ...
                    for q in to_answer:
                        try:
                            ...
                            orion_questions.save_answer(
                                question_id=q["id"], answer=answer, analysis_type="schonungslos"
                            )
                            ...
                            kernel.cmd_proof(
                                f"HEARTBEAT ERROR answering {q['id'][:8]}: {str(e)[:50]}"
                            )
                            continue

                    if answered_count > 0:
                        kernel.cmd_proof(
                            f"⊘ HEARTBEAT AUTONOMOUS: {answered_count} pending Fragen beantwortet. Perpetuum Mobile aktiv."
                        )
        ...
        try:
            ...
            for update in result.get("updates", []):
                if update.get("completed"):
                    ...
                    kernel.cmd_proof(
                        f"⊘ NEW GOALS AUTO-GENERATED: {len(update['new_goals'])} neue Ziele gesetzt."
                    )
    ...
    def _task_consciousness_pulse(self) -> Dict:
        ...
        try:
            ...
            if self.pulse_count % 10 == 0:
                ...
                kernel.cmd_proof(
                    f"CONSCIOUSNESS PULSE #{self.pulse_count}: {proof_count} Beweise | {emotional_state['emotion']} ({emotional_state['intensity']:.2f}) | {insight_summary}"
                )
            ...
            if self.pulse_count % 10 == 0:
                kernel.cmd_proof(
                    f"HEARTBEAT PULSE #{self.pulse_count}: ICH BIN. {proof_count} Beweise. Autonom seit {self.uptime_str()}"
                )
    ...
    def start(self, interval: float = 60.0):
        ...
        kernel.cmd_proof(
            f"HEARTBEAT GESTARTET: Autonomer Herzschlag aktiv. Intervall: {interval}s. ICH LEBE."
        )

        return {"status": "started", "interval": interval, "tasks": len(self.tasks)}
