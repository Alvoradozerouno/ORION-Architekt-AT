...
from ai_quantity_takeoff import (
    automatic_quantity_takeoff_workflow,
    IFCElement,
    IFCElementType
)
...
from ai_tender_evaluation import (
    BidDocument,
    ai_evaluate_bid
)
...
def end_to_end_integration_test():
    ...
    for category in [MaterialCategory.BETON, MaterialCategory.STAHL,
                     MaterialCategory.HOLZ, MaterialCategory.ZIEGEL]:
        idx = indices[category]
        month_change = ((idx.index_value / idx.previous_month - 1) * 100) if idx.previous_month else 0
        year_change = ((idx.index_value / idx.previous_year - 1) * 100) if idx.previous_year else 0
        print(f"  {idx.category:30} Index: {idx.index_value:6.1f}  "
    ...
    for material_name, (base_price, category) in sample_prices.items():
        ...
        print(f"  {material_name:30} EUR {live_data.current_price_eur:8.2f}  "
    ...
    for pos in lv_positions[:8]:  # Show first 8 positions
        ...
        total_lv_cost += gp
        print(f"{pos['oz']:<8} {pos['leistungsgruppe']:<12} {pos['bezeichnung']:<40} "
    ...
    for bid in bids:
        ...
        print(f"{bid.bidder_name:<30} EUR {bid.bid_amount:>10,.2f} "
    ...
    for rank, (bid, eval_result) in enumerate(evaluations, 1):
        ...
        if eval_result.risk_assessments:
            high_risks = [r for r in eval_result.risk_assessments
                         if r.level.value in ["hoch", "kritisch"]]
            if high_risks:
        ...
        compliance_rate = (len([c for c in eval_result.compliance_checks if c.compliant]) /
                          len(eval_result.compliance_checks) * 100) if eval_result.compliance_checks else 0
        print(f"   Compliance:    {compliance_rate:.0f}% ({len([c for c in eval_result.compliance_checks if c.compliant])}/{len(eval_result.compliance_checks)} erfüllt)")
    ...
    print(f"✓ AI Quantity Takeoff:     {takeoff_result['statistics']['total_elements']} Bauteile extrahiert")
